...
function boot(){
 const saved=new Map();const toast={textContent:'',classList:{add(){},remove(){}}};let home=true,splash=false,hidden=false;
 const context=vm.createContext({configUser:{dateArret:'2026-01-01'},window:{matchMedia:()=>({matches:true})},setTimeout(){},clearTimeout(){},localStorage:{getItem:k=>saved.get(k)||null,setItem:(k,v)=>saved.set(k,v)},document:{get visibilityState(){return hidden?'hidden':'visible'},getElementById:id=>id==='confirmation-action'?toast:id==='splash-screen'?(splash?{}:null):id==='ecran-accueil'?{classList:{contains:()=>!home}}:null,addEventListener(){}}});
...
test('celebration waits for visible home after splash; unknown color falls back safely',()=>{
 ...
 assert.equal(s.run("MyVapeUI.color('bad; background: red')"),'#ffb7c5');
});
