...
const DIYStock=(()=>{
    ...
    const labels={arome:'Arôme',base:'Base neutre',booster:'Boosters',frais:'Additif frais',sucre:'Additif sucré'};
    ...
    function plan(selection,amounts,quantity=1,rate=20){
        ...
        for(const [key,field] of Object.entries({arome:'volArome',base:'volBase',booster:'volBooster',frais:'volFrais',sucre:'volSucre'})){
            const amount=round(finite(amounts[field]??(['frais','sucre'].includes(key)?0:''),'les quantités utilisées')*quantity),id=selection?.[key];if(!id||amount===0)continue;
    ...
    function addDialog(preferred='arome'){
        ...
        const conversion=field(form,'Gouttes pour 1 ml (indication du produit, facultatif)');
        ...
        const compute=()=>{
            conversion.parentElement.hidden=!['frais','sucre'].includes(type.value);
    ...
    function render(){
        ...
        for(const [type,label] of Object.entries(labels)){
            ...
            for(const lot of entries){const card=el('article',null,'carte stock-ingredient');card.append(el('h4',lot.nom),el('p',`${ml(lot.volumeRestant)} restants sur ${ml(lot.volumeInitial)}${type==='booster'?' · '+(lot.volumeRestant/10).toLocaleString('fr-FR',{maximumFractionDigits:2})+' boosters · '+lot.tauxBooster+' mg/ml':''}`,'cout-diy-total'));
                ...
                if(lot.gouttesParMl)card.append(el('p',lot.gouttesParMl+' gouttes pour 1 ml','texte-secondaire'));
