...
html = html.replace(/<script src="push.js[^\"]*"><\/script>/, '<script src="native-notifications.js"></script><script src="native-updates.js"></script>');
...
await build({entryPoints: [path.join(root, 'native/updates.mjs')], bundle: true, format: 'iife', outfile: path.join(output, 'native-updates.js')});
